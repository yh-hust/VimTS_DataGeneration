...
if [ $idx -ge 0 ] && [ $idx -lt 4 ]; then
    input_dir="/mnt/shanghai1-only-xuwuheng-can-fucking-use/haoyan/VimTS/codef_video/codef_video_split/split_0/"
    export CUDA_VISIBLE_DEVICES=$idx # 指定要使用的GPU
    python gen.py --method "codef-based" --video_dir "$input_dir" --save_dir "$save_dir" --seed 2333
else
