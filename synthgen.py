...
from flow_functions import flows_propagation,mask2kcs
class TextRegions(object):
    ...
    min_z_projection = 0.5  # yuan 0.25
...
class RendererV10(object):
    ...
    def next_wbb(self, mask, wbb, seg_mask=None, M=None):
        ...
        return next_wbbs
    """
    def next_cbb(self,mask, cbb, seg_mask = None,M,None):
        for cbb_ in cbb:
            cbb_mask = np.zeros_like(mask)
            for cbb__ in cbb_:
                cbb_mask = cv2.fillPoly(cbb_mask, [cbb__.astype(np.int64)], 1)
            mask_ = cbb_mask * mask
            if M is not None:
                mask_ = cv2.warpPerspective(mask_, M.T, (mask_.shape[1], mask_.shape[0]))
            if seg_mask is not None:
                mask_ = mask_ * seg_mask
            next_cbb = self.mask2cbb(mask_)
            next_cbbs.append(next_cbb)
        return next_cbbs
    """
    def next_cbb(self, mask, charbb, seg_mask=None, M=None):
        next_cbbs = []
        m,_ = charbb[0].shape
        charbbs = copy.deepcopy(charbb)
        try:
            charbbs = charbbs[0].reshape(int(m/4),4,2)
        except:
            print("next_cbb wrong")
        for cbb_ in charbbs:
            cbb_mask = cv2.fillPoly(np.zeros_like(mask), [cbb_.astype(np.int64)], 1)
            mask_ = cbb_mask * mask
            if M is not None:
                mask_ = cv2.warpPerspective(mask_, M.T, (mask_.shape[1], mask_.shape[0]))
            if seg_mask is not None:
        ...
        return np.array(next_cbbs).reshape(m,2)
    ...
    def flows_propagation_torch(self, flows, seg_masks):
        ...
        for i, flow in enumerate(flows):
            ...
            pflows.append(pflow)
        return pflows[1:]
    def modified_flows_propagation_torch(self, flows, seg_masks):
        device = flows[0].device
        _, h, w = flows[0].shape
        y, x = torch.meshgrid(torch.arange(0, h), torch.arange(0, w))
        xy = torch.stack([x, y], 0).to(device)
    ...
    def place_text(self,imgs,flows, depths, segs, label, collision_mask, H, Hinv, key):
        ...
        # char_bb[0] = bb
        char_bb = copy.deepcopy(wbbs)
        key_charbb = [bb.transpose(2, 1, 0).reshape(bb.shape[-1] * bb.shape[1], bb.shape[0])]
        char_bb[0] = [key_charbb[0]]
        wbbs[0] = key_wbb
        ...
        forward_pflows = self.modified_flows_propagation_torch(forward_flows, erode_seg_masks[key:])

        backward_flows = [-el for el in flows[:key]]
        backward_flows.reverse()

        backward_pflows = self.modified_flows_propagation_torch(backward_flows, erode_seg_masks[:key + 1][::-1])
        ...
        for idx, pflow in enumerate(forward_pflows):
            ...
            if idx < 0:
                ...
                next_charbb = [np.zeros_like(el) for el in key_charbb]
            else:
                next_mask,M,flag = flows_propagation(masks[-1],pflow)
                if flag == False:
                    return
                ...
                if (next_mask > 0).sum() < min_area:
                    ...
                    next_wbb = self.next_wbb(copy.deepcopy(masks[-1]), wbbs[-1], seg_mask, M)
                    next_charbb = [self.next_cbb(copy.deepcopy(masks[-1]), char_bb[-1], seg_mask, M)]
            masks.append(next_mask)
            wbbs.append(next_wbb)
            char_bb.append(next_charbb)
        Flag = False
        for idx, pflow in enumerate(backward_pflows):
            ...
            if idx<0:
                ...
                next_mask, M,flag = flows_propagation(masks[0], pflow)
                if flag == False:
                    return
                if params['method']['shelter']:
                    seg_mask = seg_masks[key - idx - 1]
                ...
                if (next_mask > 0).sum() < min_area:
                    ...
                    next_wbb = self.next_wbb(copy.deepcopy(masks[0]), wbbs[0], seg_mask, M)
                    next_charbb = [self.next_cbb(copy.deepcopy(masks[0]), char_bb[0], seg_mask, M)]
            masks.insert(0, next_mask)
            wbbs.insert(0, next_wbb)
            char_bb.insert(0, next_charbb)
        #import pdb;pdb.set_trace()
        t_masks = []
        for mask,seg_mask in zip(masks,seg_masks):
            t_masks.append(mask*seg_mask)
        masks = t_masks
        t5 = time.time()
        if params['method']['postprocess'] == 'hw':
            ...
            wbbs = [el if is_valid else [np.array((0, 2), np.int64)] * len(el) for el, is_valid in
                    zip(wbbs, valid_wbbs)]
            char_bb = [el if is_valid else [np.array((0, 2), np.int64)] * len(el) for el, is_valid in
                       zip(char_bb, valid_wbbs)]
        ...
        return imgs_final, text, wbbs, char_bb, collision_mask
    def save(self, masks):
        import os
    ...
    def char2charBB(self,charBB,text):
        wrds = text.split()
        bb_idx = np.r_[0, np.cumsum([len(w) for w in wrds])]
        wordBB = np.zeros((2,4,len(wrds)),'float32')
        CharBB = []
        for i in range(1,len(bb_idx)):
            CharBB.append(charBB[:,:,bb_idx[i-1]:bb_idx[i]])
        return CharBB
    ...
    def render_text(self, src_imgs, flows, depths, segs, areas, labels, key):
        ...
        for idx in reg_range:
            ...
            if txt_render_res is not None:
                placed = True
                imgs, text, bb, char_bb, collision_mask = txt_render_res
        ...
        ibbs = list(zip(*ibbs))
        char_bbs = list(zip(*char_bbs))

        if placed:
            for img, ibb, char_bb in zip(imgs, ibbs,char_bbs):
